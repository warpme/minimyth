...
mm-all:
	...
	@if [ ! "$(mm_KERNEL_HEADERS_VERSION)" = "2.6.35" ] && \
	    [ ! "$(mm_KERNEL_HEADERS_VERSION)" = "2.6.36" ] ; then \
		echo "error: mm_KERNEL_HEADERS_VERSION=\"$(mm_KERNEL_HEADERS_VERSION)\" is an invalid value." ; \
		exit 1 ; \
	fi
	@echo "    mm_KERNEL_VERSION"
	@if [ ! "$(mm_KERNEL_VERSION)" = "2.6.35" ] && \
	    [ ! "$(mm_KERNEL_VERSION)" = "2.6.36" ] ; then \
	...
	@if [ ! "$(mm_NVIDIA_VERSION)" = "96.43.19"  ] && \
	    ...
	    [ ! "$(mm_NVIDIA_VERSION)" = "260.19.12" ] ; then \
