...
MYTHTV_STABLE20_FIXES_VERSION = 15332
...
mythtv-fixes-patch:
	...
	@$(call FETCH_SVN, \
		http://svn.mythtv.org/svn/branches/release-0-20-fixes/$(GARNAME), \
		$(MYTHTV_STABLE20_FIXES_VERSION),  \
		$(GARNAME)-$(MYTHTV_STABLE20_FIXES_VERSION))
	@rm -rf $(WORKDIR)/fixes-patch
	@mkdir -p $(WORKDIR)/fixes-patch
	@tar -jxf $(DOWNLOADDIR)/$(DISTNAME).tar.bz2 -C $(WORKDIR)/fixes-patch
	@tar -jxf $(PARTIALDIR)/$(GARNAME)-$(MYTHTV_STABLE20_FIXES_VERSION).tar.bz2 -C $(WORKDIR)/fixes-patch
	...
	@if [ -e files/$(DISTNAME)-fixes-$(MYTHTV_STABLE20_FIXES_VERSION).patch ] ; then \
		echo "error: cannot remove existing patch file from the files directory" ; \
		exit 1 ; \
	 fi
	@cp $(WORKDIR)/fixes-patch/$(DISTNAME)-fixes-$(MYTHTV_STABLE20_FIXES_VERSION).patch files/
	@if [ ! -w checksums ] ; then \
	...
	@md5sum files/$(DISTNAME)-fixes-$(MYTHTV_STABLE20_FIXES_VERSION).patch >> checksums
	@sed -i 's%files/\($(DISTNAME)-fixes-$(MYTHTV_STABLE20_FIXES_VERSION).patch\)%download/\1%' checksums
	@rm -rf $(WORKDIR)/fixes-patch
