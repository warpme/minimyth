...
mm-all:
	...
	@if [ ! "$(mm_KERNEL_HEADERS_VERSION)" = "3.2" ] && \
	    [ ! "$(mm_KERNEL_HEADERS_VERSION)" = "3.3" ] ; then \
		echo "error: mm_KERNEL_HEADERS_VERSION=\"$(mm_KERNEL_HEADERS_VERSION)\" is an invalid value." ; \
		exit 1 ; \
	fi
	@echo "    mm_KERNEL_VERSION"
	@if [ ! "$(mm_KERNEL_VERSION)" = "3.2" ] && \
	    [ ! "$(mm_KERNEL_VERSION)" = "3.3" ] ; then \
	...
	@if [ ! "$(mm_NVIDIA_VERSION)" = "96.43.20"  ] && \
	    [ ! "$(mm_NVIDIA_VERSION)" = "173.14.31" ] && \
	    [ ! "$(mm_NVIDIA_VERSION)" = "270.41.19" ] && \
	    [ ! "$(mm_NVIDIA_VERSION)" = "275.36"    ] && \
	    [ ! "$(mm_NVIDIA_VERSION)" = "290.10"    ] && \
	    [ ! "$(mm_NVIDIA_VERSION)" = "295.20"    ] && \
	    [ ! "$(mm_NVIDIA_VERSION)" = "295.33"    ] ; then \
