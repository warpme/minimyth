...
push(@middle,  q(  <p>));
push(@middle,  q(    This is a list of all the software built as part of the MiniMyth build process.));
push(@middle,  q(    Depending on the MiniMyth build system configuration options,));
push(@middle,  q(    some of the software may not have been included in the MiniMyth binary image.));
push(@middle,  q(  </p>));
push(@middle,  q(  <ul>));
push(@middle,  q(    <li>));
...
my %type_header = ( 'base'   => q(base ('/' and '/usr'):),
                    ...
                    'build'  => q(build (natively compiled software used for cross compiling and assembling MiniMyth):));
